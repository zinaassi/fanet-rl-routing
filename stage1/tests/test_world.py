...
def test_gs_has_no_outgoing_edges():
    w = world.build_world("random", 0.6, 0)
    assert w.raw_graph.out_degree(config.GS_ID) == 0
    assert w.graph.out_degree(config.GS_ID) == 0


def test_cutoff_removes_edges():
    # One drone in range of the GS, one far beyond the k=1.0 cutoff range.
    far = channel.max_link_range_m(1.0) * 2.0
    positions = np.array([[100.0, 0.0], [far, 0.0], [0.0, 0.0]])
    g = world.candidate_graph(positions, k=1.0)
    assert g.has_edge(0, 2)
    assert not g.has_edge(1, 2)
    assert not g.has_edge(0, 1)  # drone-drone edges obey the same cutoff


def test_candidate_edges_match_cutoff_exactly():
    w = world.build_world("random", 0.6, 4)
    pos = w.positions
    for i in range(config.N_DRONES):
        for j in range(config.N_DRONES + 1):
            if i == j:
                continue
            pl = float(channel.p_loss(_dist(pos[i], pos[j]), 0.6))
            assert w.raw_graph.has_edge(i, j) == (pl <= config.P_LOSS_CUTOFF)
...
def test_prune_caps_drone_to_drone_out_degree():
    w = world.build_world("random", 0.3, 2)  # long range => dense raw graph
...
def test_prune_keeps_lowest_ploss_edges():
    w = world.build_world("random", 0.3, 2)
    for u in range(config.N_DRONES):
        kept = {v for v in w.graph.successors(u) if v != config.GS_ID}
        all_edges = sorted(
            ((d["p_loss"], v) for v, d in w.raw_graph[u].items() if v != config.GS_ID),
        )
...
def test_gs_edge_exempt_from_cap():
    # Drone 0 sits 250 m from the GS with six much closer drone neighbours.
    # Without the exemption the GS edge would lose the top-5 contest.
    d0 = np.array([250.0, 0.0])
    nbrs = [d0 + [10.0 + i, 5.0] for i in range(6)]
    positions = np.array([d0] + nbrs + [[0.0, 0.0]])
    pruned = world.prune_out_edges(world.candidate_graph(positions, k=0.3))
    gs = len(positions) - 1
...
def test_prune_tie_break_by_neighbor_id():
    ...
    pruned = world.prune_out_edges(world.candidate_graph(positions, k=1.0))
...
def test_prune_disconnect_detection():
    ...
    cluster_center = np.array([1300.0, 0.0])
    offsets = [(0, 0), (20, 0), (-20, 0), (0, 20), (0, -20), (15, 15)]
    cluster = [cluster_center + off for off in offsets]
    positions = np.array(cluster + [[600.0, 0.0], [0.0, 0.0]])  # bridge=6, GS=7
    raw = world.candidate_graph(positions, k=0.3)
...
def test_build_world_reports_prune_disconnects():
    w = world.build_world("random", 1.0, 9)
